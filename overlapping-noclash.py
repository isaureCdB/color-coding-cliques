...
def detect_clashes_COM(coor1, coor2, pairs_to_eval, homo=False):
    ...
    eval_key = len(coor2_com) * pairs_to_eval[:, 1] + pairs_to_eval[:, 0]
    eval_key = np.sort(eval_key)
    pairs_to_keep = np.intersect1d(pairs_key, eval_key) #, assume_unique=True)
    print("PAIRS BEFORE EVAL", pairs, pairs.shape, len(coor2_com))
...
def connection_1nucl(g, connect2nucl):
...
connect1nucl = connection_1nucl(g, connect2nucl) - 1
