...
print chains.shape
monomers_chains = [ chains[:, atoms_aa[i]:atoms_aa[i+1] ,:] for i in range(Nres) ]
...
for j in range(len(chains)):
    chain = chains[j]
    for i in r:
        monomer = chain[atoms_aa[i]:atoms_aa[i+1],:]
        fitted, RMSD = fit_another_multi_npy(libraries_aa[i], libraries_aa[i], monomer)
